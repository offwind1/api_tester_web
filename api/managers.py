...
class ApiManager(models.Manager):
    ...
    def update(self, belong_module, api_name, request, id=None):
        if id:
            api = self.filter(id=id).first()

            if api:
                api.belong_module = belong_module
            ...
            self.create(belong_module=belong_module, api_name=api_name, request=request)
            return "/api/api_list/"
